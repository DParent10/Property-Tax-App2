...
namespace Property_Tax
{
    public partial class Form1 : Form
    {
        ...
        private void CalculateOccupancyPhysicalValue()
        {
            try
            {
                // Parse the representative value
                if (double.TryParse(occupancyrepval1.Text, out double repValue))
                {
                    // Parse the physical depreciation percentage
                    // Assuming the user enters values like "10" for 10%
                    if (double.TryParse(buildinginfophysdeppercent1.Text, out double physDepPercent))
                    {
                    ...
                    {
                        // Handle invalid input in physical depreciation box
                        MessageBox.Show("Invalid physical depreciation percentage.");
                        occupancyphysval1.Text = string.Empty;
                    }
                }
                else
                {
                    // Handle invalid input in representative value box
                    MessageBox.Show("Invalid representative value.");
                    occupancyphysval1.Text = string.Empty;
                }
            }
            catch (Exception ex)
